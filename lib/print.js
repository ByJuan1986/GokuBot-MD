...
let name_user
let _name = await conn.getName(m.sender) || 'Sin registro.'
...
return chalk.bold(`${name_info ? name_info : 'Sin registro.' }`)
}).join(', ')

console.log(`
╭─────────🜲 MBMD 🜲─────────⋆
│⫹⫺ ${chalk.cyan.bold('Bot:')} ${chalk.green.bold('%s')}
│⧖ ${chalk.cyan.bold('Hora:')} ${chalk.black.bgGreen('%s')}
│⎙ ${chalk.cyan.bold('Usuario:')} ${chalk.green('%s')}
│ %s
│⎋ ${chalk.cyan.bold('Mensaje:')} ${chalk.bgGreenBright.bold('[%s]')}\n%s
╰─────────🜲 MBMD 🜲─────────⋆`.trim(),
...
sender,
m ? '' : '',
`${user.level} / ${user.boletos}`,
m.chat.includes('@s.whatsapp.net') 
? `${chalk.cyan.bold('Privado:')} ${chalk.greenBright(`${m.sender} ~${user.name || conn.getName(m.sender)}`)}`
: m.chat.includes('@g.us') 
? `${chalk.cyan.bold('Grupo:')} ${chalk.greenBright(`${m.chat} ~${chat}`)}`
: m.chat.includes('@newsletter') 
? `${chalk.cyan.bold('Canal:')} ${chalk.greenBright(`${m.chat} ~${chat}`)}`
: '',
filesize,
filesize === 0 ? 0 : (filesize / 1009 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1),
['', ...'KMGTP'][Math.floor(Math.log(filesize) / Math.log(1000))] || '',
await formatMessageTypes(m.mtype) : await formaTxt(m.mtype) || 'Not specified', //m.mtype ? m.mtype : 'No especificado'  //? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : ''
